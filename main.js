...
function initializeGame(doc = document) {
  ...
  deliveredListEl = doc.getElementById('delivered-list');
  storageListEl = doc.getElementById('storage-list');
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (e.key === 's' || e.key === 'S') setSelected('storage');
...
function placeStructure(x, y, type, rotation) {
  ...
  if (type === 'belt') {
    ...
    state.structures.set(key(x, y), { type: 'storage', items: [], capacity: 20 });
  }
...
function addToStorage(storage, item) {
  if (storage.items.length >= storage.capacity) return false;
...
function trackDelivered(type) {
  state.deliveredCounts[type] = (state.deliveredCounts[type] || 0) + 1;
}
...
function updateBelts(dt) {
  ...
  transfers.forEach((transfer) => {
    ...
    if (to && to.type === 'belt') {
      ...
      trackDelivered(item.type);
    } else if (to && to.type === 'storage') {
      const stored = addToStorage(to, item);
      if (!stored) {
        originBelt.items.push({ ...item, progress: 0.95 });
      }
...
function summarizeStorage() {
  ...
  state.structures.forEach((structure) => {
    if (structure.type !== 'storage') return;
...
function refreshPanels() {
  if (!deliveredListEl || !storageListEl) return;
  const deliveredEntries = Object.entries(state.deliveredCounts);
  deliveredListEl.innerHTML = deliveredEntries.length
    ? deliveredEntries
        .map(([type, count]) => `<li><span>${type.toUpperCase()}</span><span>${count}</span></li>`)
        .join('')
    : '<li><span>Nothing yet</span><span>—</span></li>';

  const storageSummary = summarizeStorage();
  const storageEntries = Object.entries(storageSummary);
  const totalStored = storageEntries.reduce((sum, [, count]) => sum + count, 0);
  const capacity = totalStorageCapacity();

  if (capacity === 0) {
    storageListEl.innerHTML = '<li><span>No crates placed</span><span>—</span></li>';
    return;
  }

  const capacityRow = `<li><span>Used</span><span>${totalStored}/${capacity}</span></li>`;
  const detailRows = storageEntries
    .map(([type, count]) => `<li><span>${type.toUpperCase()}</span><span>${count}</span></li>`)
    .join('');
  storageListEl.innerHTML = capacityRow + (detailRows || '<li><span>Empty</span><span>—</span></li>');
}
...
function drawStructures() {
  state.structures.forEach((structure, pos) => {
    ...
    if (structure.type === 'storage') drawStorage(x, y, structure.items, structure.capacity);
  });
}
...
function drawStorage(x, y, items, capacity) {
  ctx.fillStyle = colors.storage;
  ctx.fillRect(x * tileSize + 3, y * tileSize + 3, tileSize - 6, tileSize - 6);
  ctx.strokeStyle = '#0f292c';
  ctx.lineWidth = 2;
  ctx.strokeRect(x * tileSize + 3, y * tileSize + 3, tileSize - 6, tileSize - 6);

  const fill = Math.min(1, items.length / capacity);
  ctx.fillStyle = 'rgba(255,255,255,0.12)';
  ctx.fillRect(x * tileSize + 6, y * tileSize + 6 + (1 - fill) * (tileSize - 12), tileSize - 12, (tileSize - 12) * fill);

  ctx.fillStyle = '#d8f1ff';
  ctx.font = '12px Inter';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText('ST', (x + 0.5) * tileSize, (y + 0.38) * tileSize);
  ctx.fillStyle = '#9cd5e1';
  ctx.font = '11px Inter';
  ctx.fillText(`${items.length}/${capacity}`, (x + 0.5) * tileSize, (y + 0.65) * tileSize);
}
